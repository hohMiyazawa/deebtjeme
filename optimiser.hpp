...
//lz interlude
	...
	double synth = synthness_grey(in_bytes, width, height);
	printf("synthness: %f\n",synth);
	if(synth > 0.4){
		printf("Trying LZ\n");
		float* finalCost = new float[width*height];
		for(size_t index=width*height;index--;){
			...
			finalCost[index] =
				-std::log2(
					(
						(double)(table[entropy_image[tile_index]].freqs[filtered_bytes[index]])
					)/((double)(1 << 16))
				);
		}
		lz_data = lz_dist_grey(
		...
		if(lz_size > 1){
			...
			for(size_t index=width*height;index--;){
				...
				statistics[entropy_image[tile_index]].freqs[filtered_bytes[index]]++;
			}
