...
*/
	...
	for(size_t i=1;i<lz_size;i++){
		stats_backref.freqs[reval[i].backref]++;
		stats_matchlen.freqs[reval[i].matchlen]++;
		stats_future.freqs[reval[i].future]++;
	}
	stats_future.freqs[reval[0].future]++;

/*
	for(size_t i=0;i<256;i++){
		printf("vval %d\n",(int)stats_backref.freqs[i]);
	}
*/
