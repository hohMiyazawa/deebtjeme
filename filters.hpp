...
uint8_t* filter_all_sPaeth(uint8_t* in_bytes, uint32_t range, uint32_t width, uint32_t height){
	uint8_t* filtered = new uint8_t[width * height];
	filtered[0] = in_bytes[0];//TL prediction
	if(range == 256){
		for(size_t i=1;i<width;i++){
...
uint8_t* filter_all_median(uint8_t* in_bytes, uint32_t range, uint32_t width, uint32_t height){
	uint8_t* filtered = new uint8_t[width * height];
...
uint8_t* filter_all(uint8_t* in_bytes, uint32_t range, uint32_t width, uint32_t height,uint8_t predictor){
	if(predictor == 0){
		...
		return filter_all_median(in_bytes, range, width, height);
	}
	else if(predictor == 7){
		return filter_all_sPaeth(in_bytes, range, width, height);
	}
