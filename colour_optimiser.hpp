...
void colour_optimiser_subColour(
	uint8_t* in_bytes,
	uint32_t range,
	uint32_t width,
	uint32_t height,
	...
	uint8_t* filtered_bytes = colour_filter_all_ffv1_subColour(in_bytes, range, width, height, rg, bg, br);
	...
	filter_collection[0] = colour_filter_all_ffv1_subColour(in_bytes, range, width, height, rg, bg, br);
...
0b0011010011000001,
	...
	for(size_t i=1;i<available_predictors;i++){
		predictorCount = colourSubColour_add_predictor_maybe_prefiltered(
			in_bytes,
			filtered_bytes,
			range,
			width,
